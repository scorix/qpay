describe Qpay do
  describe "#sign" do
    ...
    context :params_without_noncestr do
      let(:params) { {device_info: '1000', body: 'test', appid: 'wxd930ea5d5a258f4f', mch_id: '10000100'} }

      context :sign do
        subject { Qpay.sign(params, config, 'ibuaiVcKdpRxkhJA') }
        it { is_expected.to eql '9A0A8659F005D6984697E2CA0A9CF3B7' }
      end

      context :string_params_without_sign do
        subject { Qpay.string_params_without_sign(params, config, 'ibuaiVcKdpRxkhJA') }
      ...
      context :params_with_sign do
        subject { Qpay.params_with_sign(params, config, 'ibuaiVcKdpRxkhJA') }
